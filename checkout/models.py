...
class Order(models.Model):
    ...
    def update_total(self):
        ...
        self.delivery_cost = (
            self.order_total * settings.BEGINNER_DELIVERY_CHARGE / 100
            )
        self.discount = (
            self.order_total * settings.BEGINNER_STORE_DISCOUNT / 100
            )
        self.grand_total = (
    ...
    def save(self, *args, **kwargs):
        ...
        if self.user_profile:
            if self.user_profile.membership_type == 'Pro':
                self.delivery_cost = (
                    self.order_total * settings.PRO_DELIVERY_CHARGE / 100
                )
                self.discount = (
                    self.order_total * settings.PRO_STORE_DISCOUNT / 100
                )
                self.grand_total = (
                    self.order_total - self.discount + self.delivery_cost
                )
